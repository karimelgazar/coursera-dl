...
from cs_dlp.libs.api import CourseraOnDemand, ItemsV2, LessonsV1, ModulesV1, OnDemandCourseMaterialItemsV1
from cs_dlp.libs.define import OPENCOURSE_ONDEMAND_COURSE_MATERIALS_V2
from cs_dlp.libs.network import get_page
from cs_dlp.libs.utils import is_debug_run, spit_json
...
class CourseraExtractor(PlatformExtractor):
    ...
    def list_courses(self):
        ...
        @rtype: [str]
        """
        course = CourseraOnDemand(session=self._session, course_id=None, course_name=None)
    ...
    def _parse_on_demand_syllabus(
        ...
        logging.info(
            "Parsing syllabus of on-demand course (id=%s). This may take some time, please be patient ...", class_id
        )
        modules = []
                ...
                for lecture in available_lectures:
                    ...
                    if typename == "lecture":
                        ...
                        links = course.extract_links_from_lecture(
                            class_id, lecture_video_id, subtitle_language, video_resolution
                        )
                        ...
                        links = course.extract_links_from_ungradedWidget(lecture.id)
